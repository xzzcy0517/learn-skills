...
| 上游情况 | 处理方式 |
| --- | --- |
| 有开源协议(MIT / Apache-2.0 等) | ✅ 正常快照,按协议保留 LICENSE |
| 无任何协议声明 | ✅ 可快照(视同作者未做开源管理,非拒绝使用);SOURCE.yml 标 `license: 未声明`,笔记与 SNAPSHOT.md 中醒目标注出处与版权归属,**作者提出异议时立即删除快照** |
| 明示限制(Proprietary / 服务条款约束 / 私有仓库) | ❌ 绝不快照,只写笔记 + 跳转出处,文首做版权提示 |

拿不准的按更严一档处理。本目录内容版权归各上游原作者与项目所有,按各自许可条款分发;
